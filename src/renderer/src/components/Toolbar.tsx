...
function ToolButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "text-foreground/80 hover:bg-accent hover:text-foreground rounded-md p-1.5 disabled:pointer-events-none disabled:opacity-35",
...
export function Toolbar(): React.JSX.Element {
  ...
  return (
    <header className="bg-sidebar flex h-11 shrink-0 items-center gap-1 border-b pr-3 pl-20 [-webkit-app-region:drag]">
      ...
      />
      <div className="flex-1 truncate text-center text-[13px] font-semibold">{title}</div>
      <ToolButton
        icon={RotateCw}
        label="Refresh"
